...
; H2 server mode
(def h2-server-db-spec {:classname   "org.h2.Driver"
                        :subprotocol "h2"
  ...
  (cond
    (or (= (:subprotocol db-spec) "h2")
        (and (:datasource db-spec)
             (re-find #"^jdbc:h2:" (.getJdbcUrl (:datasource db-spec)))))
    ...
    (or (= (:subprotocol db-spec) "hsqldb")
        (and (:datasource db-spec)
             (re-find #"^jdbc:hsqldb:" (.getJdbcUrl (:datasource db-spec)))))
    ...
    (or (= (:subprotocol db-spec) "mysql")
        (and (:datasource db-spec)
             (re-find #"^jdbc:mysql:" (.getJdbcUrl (:datasource db-spec)))))
...
(defn h2-database-initialized?
  []
  (.exists (new java.io.File (str "./" db-name ".h2.db"))))
...
(defn create-indexes
  [db-spec]
  (try (sql/db-do-commands db-spec "CREATE INDEX board_info_index                   ON board_info        ( board(128)        );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX thread_info_index                  ON thread_info       ( thread_no         );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX bookmarks_index                    ON bookmarks         ( thread_no         );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX bookmarks_board_index              ON bookmarks         ( board(128)        );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX dat_files_index                    ON dat_files         ( thread_no         );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX threads_in_html_index              ON threads_in_html   ( thread_no         );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX threads_in_json_index              ON threads_in_json   ( thread_no         );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX favorite_threads_index             ON favorite_threads  ( thread_no         );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX images_index                       ON images            ( url(128)          );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX downloads_index                    ON downloads         ( url(128)          );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX user_settings_index                ON user_settings     ( setting_name(128) );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX system_settings_index              ON system_settings   ( setting_name(128) );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX post_filters_patern_index          ON post_filters      ( pattern(128)      );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX post_filters_board_index           ON post_filters      ( board(128)        );") (catch Throwable t))
  (try (sql/db-do-commands db-spec "CREATE INDEX images_extra_info_md5_string_index ON images_extra_info ( md5_string(128)   );") (catch Throwable t)))
